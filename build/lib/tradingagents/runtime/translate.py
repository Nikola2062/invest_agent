"""Translate report/digest markdown to Traditional Chinese (繁體中文) via DeepSeek.

Needs DEEPSEEK_API_KEY (passed via --deepseek-key). Callers fall back to English
when ``available()`` is False. Best-effort: a failed chunk keeps its English.
Ported from investor_agent/orchestrator/translate.py.
"""
...
_SYS_ZH = (
    ...
    "percentages, dates, currency codes (USD/HKD).\n"
    "3. Keep common financial acronyms in English: VIX, CAPE, P/E, OAS, ETF, HSI, YoY, RS, PMI.\n"
...
def to_traditional_chinese(md: str) -> str | None:
    """Translate markdown to 繁體中文, or None if no DeepSeek key / SDK."""
    client = _client()
    if client is None:
        return None
    model = os.environ.get("DEEPSEEK_MODEL_DEFAULT", "deepseek-chat")
    out = []
    chunks = _chunks(md)
    for i, ch in enumerate(chunks, 1):
        try:
            resp = client.chat.completions.create(
                ...
                messages=[{"role": "system", "content": _SYS_ZH},
                          {"role": "user", "content": ch}],
            )
            out.append(resp.choices[0].message.content)
        except Exception as e:
            print(f"[translate] chunk {i}/{len(chunks)} failed ({e}); keeping English",
                  file=sys.stderr)
            out.append(ch)
    return "\n".join(out)
